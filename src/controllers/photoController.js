...
const loadUncensorPhotoPage = async (req, res) => {
    let dataphoto = await photoService.getAllUncensorPhotoByDepartmentId(req.id_department);
    let page = "pages/photo_index.ejs";
    let title = "Quản lí hình ảnh của phòng ban";
    let modal = { title: "Xóa hình ảnh", objectName: "hình ảnh", formAction: "/photo/delete" };
    let type = "censor";
...
const updatePhoto = async (req, res) => {
    ...
    let id = req.body.id;
    if (req.user_role != 1) {
        if (await photoService.checkPhotoDependent(id, null, req.id_department) == false) {
            req.flash('error', 'Ảnh không thuộc quyền sở hữu của phòng ban');
            return res.redirect('back');
        }
    }
    let dataPhoto = {
...
const deletePhotoByUser = async (req, res) => {
    ...
    if (await photoService.checkPhotoDependent(id, req.user_id, null) == false) {
        req.flash('error', 'Ảnh không thuộc quyền sở hữu của bạn');
        return res.redirect('/user/photo');
    }
